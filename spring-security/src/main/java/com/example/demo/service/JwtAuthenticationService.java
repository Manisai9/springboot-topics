...
public class JwtAuthenticationService {
	...
	public String generateToken(Users users) {
		...
		String token = Jwts.builder().setSubject("" + users.getId()).setClaims(getClaims(users))
	...
	private Map<String, Object> getClaims(Users users) {
		logger.info("Mapping jwtRequest to token creation content...");
		Map<String, Object> claims = new HashMap<>();
		claims.put("typeOfUser", users.getRole().toString());
		claims.put("id", users.getId());
		claims.put("emailId", users.getEmail());
		if (!StringUtils.isBlank(users.getFullName())) {
